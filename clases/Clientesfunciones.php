...
{
    $sql = $con->prepare("INSERT INTO clientes(nombre, apellidos, email, telefono, dni, estatus, fecha_alta) VALUES(?,?,?,?,?, 1, now())");
...
function emailExiste($email, $con){
    ...
    $sql = $con->prepare("SELECT email FROM clientes WHERE email LIKE ? LIMIT 1");
    $sql->execute([$email]);
